...
export function setupWebSocket(httpServer: HTTPServer) {
	...
	function startQuestion() {
		...
		console.log(
			`Starting question ${currentQuestionIndex}: ${question.question}`,
		);
	...
	io.on("connection", (socket) => {
		...
		socket.on("answer_submit", (data) => {
			...
			console.log(
				`${player.name} answered: ${answer} (${isCorrect ? "correct" : "wrong"})`,
			);
			...
			if (isCorrect) {
				...
				console.log(
					`Hydra took ${damage} damage! Health: ${gameState.hydraHealth}/${gameState.maxHydraHealth}`,
				);
			} else {
