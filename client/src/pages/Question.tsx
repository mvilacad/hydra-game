...
export default function Question({ question, onAnswer, timeLeft }: QuestionProps) {
  ...
  const getAttackTypeInfo = (type: string) => {
    switch (type) {
      case 'sword':
        return { name: 'Golpe de Espada', color: 'text-orange-400', icon: '⚔️' };
      case 'arrow':
        return { name: 'Tiro de Flecha', color: 'text-green-400', icon: '🏹' };
      case 'magic':
        return { name: 'Feitiço Mágico', color: 'text-purple-400', icon: '✨' };
      case 'fire':
        return { name: 'Ataque de Fogo', color: 'text-red-400', icon: '🔥' };
      default:
        return { name: 'Ataque', color: 'text-gray-400', icon: '⚡' };
    }
  };
        ...
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xl text-white">Prepare Seu Ataque!</h3>
              ...
              <div className={cn(
                ...
                {isCorrect ? (
                  <>
                    ...
                    Excelente! Seu ataque acertou o alvo!
                  </>
                ) : (
                  <>
                    <XCircle className="w-5 h-5" />
                    Seu ataque errou o alvo...
                  </>
                )}
              </div>
            </div>
        ...
        <Card className="bg-gray-800/30 backdrop-blur-sm border-gray-600/30">
          <CardContent className="p-4 text-center">
            <p className="text-gray-300 text-sm">
              Escolha sua resposta para lançar um{' '}
