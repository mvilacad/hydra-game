...
export function BattleHUD({ gameState, isConnected, className }: BattleHUDProps) {
  const { 
    phase, 
    players, 
    hydraHealth, 
  ...
  const averageScore = players.length > 0 ? players.reduce((sum, p) => sum + p.score, 0) / players.length : 0;
      ...
      <Card className="bg-gray-900/95 border-gray-700/50 backdrop-blur-sm">
        <CardContent className="p-4">
          <div className="flex items-center gap-3 mb-4">
            <div className={cn("p-2 rounded-lg border", phaseInfo.bgColor, phaseInfo.borderColor)}>
              <div className={phaseInfo.color}>
                {phaseInfo.icon}
              </div>
            </div>
            <div>
              <h3 className="font-bold text-white text-lg">Status da Batalha</h3>
              <p className={cn("font-medium", phaseInfo.color)}>{phaseInfo.label}</p>
            </div>
          </div>
          ...
          <div className="mb-4">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-sm text-white font-mono">
                {hydraHealth.toLocaleString()} / {maxHydraHealth.toLocaleString()}
              </span>
            </div>
            <div className="w-full h-3 bg-gray-800 rounded-full overflow-hidden border border-gray-600">
              <div
                className={cn(
                  ...
                  healthPercent > 60 ? "bg-gradient-to-r from-red-500 to-red-600" :
                  healthPercent > 30 ? "bg-gradient-to-r from-orange-500 to-red-500" :
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-400">Rodada {currentQuestion.round || 1}</p>
              </div>
            </div>
            ...
            <div className="relative w-16 h-16 mx-auto mb-3">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-xl font-bold text-white">{questionTimeLeft}</span>
              </div>
            </div>
