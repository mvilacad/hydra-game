import { Player, Attack } from '@shared/types';
import { Card, CardContent, CardHeader, CardTitle } from './card';
import { cn } from '@/lib/utils';
import { Sword, Target, Zap, Shield, Flame, TrendingUp, Clock, Award } from 'lucide-react';
...
export function DamageMeter({ players, attacks = [], className }: DamageMeterProps) {
  const [sortBy, setSortBy] = useState<'damage' | 'dps' | 'accuracy'>('damage');
  
  // Calculate comprehensive player stats
  const playerStats = useMemo(() => {
    const stats: PlayerStats[] = players.map(player => {
      const playerAttacks = attacks.filter(attack => attack.playerId === player.id);
      const totalDamage = playerAttacks.reduce((sum, attack) => sum + attack.damage, 0);
      ...
      const timeSpan = firstAttack && lastAttack ? (lastAttack - firstAttack) / 1000 : 1;
      ...
      const accuracy = playerAttacks.length > 0 ? (hits / playerAttacks.length) * 100 : 0;
      
      // Critical hits (high damage attacks)
      const avgDamage = hits > 0 ? totalDamage / hits : 0;
      const criticalHits = playerAttacks.filter(attack => attack.damage > avgDamage * 1.5).length;
  ...
  const getCharacterColor = (character: string) => {
    switch (character) {
      case 'warrior': return 'text-orange-400 bg-orange-400/10 border-orange-400/20';
      case 'mage': return 'text-blue-400 bg-blue-400/10 border-blue-400/20';
      case 'archer': return 'text-green-400 bg-green-400/10 border-green-400/20';
      case 'paladin': return 'text-yellow-400 bg-yellow-400/10 border-yellow-400/20';
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isTop3 && "bg-gradient-to-r from-gray-800/60 to-gray-700/60 border-yellow-400/20",
              ...
              <div className="relative flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 text-xs text-gray-400">
                    ...
                    {stat.criticalHits > 0 && (
                      <span className="text-yellow-400">{stat.criticalHits} crits</span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex-shrink-0 text-right">
                  <div className={cn(
                    ...
                    {sortBy === 'damage' && `${stat.totalDamage.toLocaleString()}`}
