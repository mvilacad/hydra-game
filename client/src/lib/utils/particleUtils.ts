...
export function createParticleGeometry(config: ParticleConfig, position: THREE.Vector3): {
  ...
  for (let i = 0; i < config.count; i++) {
    ...
    velocities[i3 + 2] = Math.sin(phi) * Math.sin(theta) * speed * config.spread;
...
export function updateParticles(
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (elapsed < lifetime) {
      ...
      positions[i3 + 1] += velocities[i3 + 1] * delta + config.gravity * delta * elapsed;
...
export function createParticleMaterial(config: ParticleConfig): THREE.PointsMaterial {
