...
export const ResultsScreen: React.FC<ResultsScreenProps> = ({
  ...
  return (
    <div className={cn("h-screen relative overflow-hidden flex items-center justify-center p-4", theme.bgPattern, className)}>
      {/* Background Effects */}
      <div className="absolute inset-0 opacity-20">
        <div className={cn("absolute top-20 left-20 w-48 h-48 rounded-full blur-3xl", `bg-gradient-to-br ${theme.primary}`)} />
        <div className={cn("absolute bottom-20 right-20 w-64 h-64 rounded-full blur-3xl", `bg-gradient-to-tl ${theme.primary}`)} />
      </div>
      ...
      <Card className={cn("w-full max-w-md backdrop-blur-xl border-2 relative z-10", `bg-gradient-to-br ${theme.secondary}`, theme.border, theme.glow, "shadow-2xl")}>
        <CardContent className="p-8 text-center">
          ...
          {/* Player Stats */}
          <div className={cn("p-4 rounded-xl mb-6", `bg-gradient-to-r ${theme.primary}`, "bg-opacity-20 backdrop-blur-sm")}>
              ...
              {sortedPlayers.slice(0, 3).map((player, index) => (
                ...
                >
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <div className={cn("text-xs capitalize", theme.accent)}>{player.character}</div>
                  </div>
          ...
          {/* Battle Status */}
          <div className="mt-6">
            <div className={cn("inline-flex items-center gap-2 px-4 py-2 rounded-full", `bg-gradient-to-r ${theme.primary}`, "animate-pulse")}>
