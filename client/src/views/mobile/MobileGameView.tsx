...
import { useBattlePhases, useAnswerSubmission, PreparationScreen, ResultsScreen } from '@/features/battle';
...
export const MobileGameView: React.FC = () => {
  const [currentPlayer, setCurrentPlayer] = useState<{ name: string; character: string; id?: string } | null>(null);
  ...
  const { battleState, transitionToPhase, startPreparationPhase, resetBattle } = useBattlePhases();
  ...
  useEffect(() => {
    if (currentPlayer && !currentPlayer.id && players.length > 0) {
      const matchingPlayer = players.find(p =>
        p.name === currentPlayer.name && p.character === currentPlayer.character
      );

      if (matchingPlayer?.id) {
        setCurrentPlayer(prev => prev ? { ...prev, id: matchingPlayer.id } : null);
      }
    }
  ...
  }, [battleCurrentQuestion, currentPlayer, currentQuestion, battleState.phase, transitionToPhase]);

  // Handle battle phase transitions
  useEffect(() => {
    if (battlePhase === 'battle' && currentPlayer && battleState.phase === 'waiting') {
  ...
  }, [battlePhase, currentPlayer, battleState.phase, startPreparationPhase, transitionToPhase]);
  ...
  const renderCurrentPhase = () => {
    switch (battleState.phase) {
      ...
      case 'question':
        if (!currentQuestion) {
          return <LoadingScreen title="Carregando Pergunta..." message="Aguarde um momento..." />;
        }
